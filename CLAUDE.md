...
- **Package migration: tsoa → `tsoa-next` (or a tsoa release with the `ignore` option).** Classic tsoa `7.0.0-alpha.0` resolves every model name reachable from controllers, and `generated/prisma/client.ts` re-exports all Prisma enums (`export * from "./enums"`). Any `api-shared/type/` union whose name matches a Prisma enum can then collide with `Found 2 different model definitions` once tsoa's type walk reaches the Prisma client (e.g. through `AuditLogger`). That is why `profileVisibility` / `jobSearchStatus` are inlined string literals in `UserDetailResponse.ts` instead of shared type files, while `JobStatus` / `UserStatus` survive as shared files. `tsoa-next` adds `ignore?: string[]` ("Directories to ignore during TypeScript metadata scan"). After migrating, restore the shared type files and set `"ignore": ["**/generated/prisma/**"]` in `tsoa.json`.
